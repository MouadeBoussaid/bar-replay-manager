...
| Scope | Rule |
|---|---|
| `s<N>` | in BAR season `N` — `start <= startTime < nextSeason.start` (see [`shared/seasons.ts`](../src/shared/seasons.ts)) |
| `all` | everything |
| `last50` | last 50 appearances |

The selector lists every season newest-first, then All time and Last 50, and
**defaults to the current season** (`CURRENT_SEASON`). Add a season by prepending
it to `SEASONS`; the oldest entry has `start: null` (open-ended past).
...
One row per map, most-played first (up to 6, min **4 positioned games** —
`START_MAP_MIN_GAMES` / `START_MAP_LIMIT`). Each row = the map's real minimap
texture with a dot per spot (community name or rank number), next to a ranked
list of that map's spots.
...
Per map (`clusterStartSpots`), in this order:

1. **Named games group by name.** Any game whose position resolves to a
   community name (see §5b · Community names) goes straight into that name's
   bucket — one stat point per name, no matter how far apart the games are. This
   is what pulls the two mirror halves of a position (and any nearby deploy
   spots the player splits between) into a single point.
2. **The rest are clustered by proximity.** Greedy assignment to the nearest
   existing cluster within `SPOT_MERGE_DIST = 0.055` normalised units (recentred
   on its running mean as points join), then up to 6 merge passes combining any
   two clusters that drifted within that distance.
3. **Mirror fold** (curated maps only — those with a `map-names` entry): before
   step 2, every point in the top half of the map is rotated 180° about the
   centre (`x,y → 1−x, 1−y`), so an un-named spot and its mirror land in one
   cluster. Named buckets are folded the same way for their plotted centroid.
4. **Floor:** an un-named spot needs `START_SPOT_MIN_GAMES = 3` games to earn a
   point; named spots are always kept. Then sort by games, descending.

On a symmetric team map this yields ~8 points (16 spawns ÷ 2 mirror).

### What each row shows

| Element | Meaning |
|---|---|
| dot position | spot centroid (mirror-folded on curated maps), as a % of map width/height |
...
| dot label | the **community name** on a curated spot (a tag beside the dot, stacked when two coincide); otherwise the spot's rank number (ties it to the list) |
| list label | community name → BAR **role** (`air`, `front/tech`, …) → `Spot N`, first that applies. Role = `majorityRole` of the spot's games (see §5b · roles) |
...
A hand-maintained layer on top of the generic roles, in
[`src/main/map-names.data.json`](../src/main/map-names.data.json) (keyed by
version-stripped lower-case map name; each entry `[x, z, name]` in world elmos).
The metagame calls certain spots by their own names — and often *asymmetrically*,
where two mirror-image positions get different names (Isthmus: `Pond` one side,
`Cove sea` the other). So it is never auto-derived — no fetch script.

A position takes a name when it is within **1000 elmos** of a named anchor
(`nameForPosition`, tighter than the role snap so an un-named neighbour keeps its
role). This resolves per game, then feeds the name-bucket grouping above. Only
the Start-positions card uses it; the Appearances "Pos" column and Overview pips
stay on the role. Currently mapped: **Supreme Isthmus**, **All That Glitters**.
...
startSpot         = games grouped by community name, else proximity cluster of a
                    player's startNX/NY on one map (< 0.055); mirror-folded on
                    curated maps; un-named spots need >= 3 games
...
spotName          = community name of the nearest named anchor (< 1000 elmos)
...
| Start positions: min positioned games / maps shown | 4 / top 6 |
| Start spot cluster / merge distance | 0.055 normalised map units |
| Start spot: min games for an un-named point | 3 (`START_SPOT_MIN_GAMES`) |
| Role snap distance (position → nearest labelled spawn) | 2600 elmos |
| Community-name snap distance (position → nearest named anchor) | 1000 elmos |
| Maps with curated role data | ~64 (`map-roles.data.json`) |
| Maps with community start-spot names | Supreme Isthmus, All That Glitters (`map-names.data.json`) |
| Company: min shared games / shown | 15 / top 4 |
| Stats snapshot period (engine) | 15 s |
| Season boundaries | `shared/seasons.ts` (S3 from 2026-01-08; default scope = current) |
